...
describe "NavigationHelper Plugin: Navigation class" do
  ...
  module NavigationSpecHelper
    ...
    def nav_with_replacement(options={})
      N.new(@replace_links, options.merge(:replace_text => true))
    end
  end
  ...
  describe "behavior" do
    ...
    describe "replace link text" do
      before(:each) do
        @replace_links = [:one, 'A', :two, 'B', :three, 'C']
        @replace_navigation = nav_with_replacement
      end

      it "should not want replace text" do
        @navigation.wants_replace_text?.should eql(false)
      end

      it "should want replace text" do
        @replace_navigation.wants_replace_text?.should eql(true)
      end

      it "should replace text" do
        links = @replace_navigation.sections
        text = @replace_links.reject{|l| l.is_a?(Symbol)}.reverse

        links.each do |link|
          @replace_navigation.text_for(link).should eql(text.pop)
        end
      end

      it "should want subtitles even if replace text is explicitly set to false" do
        nav = nav_with_subtitles(:replace => false)
        nav.wants_replace_text?.should eql(false)
        nav.wants_subtitles?.should eql(true)
      end
    end
